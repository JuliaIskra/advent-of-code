...
@main def main: Unit = {
  println(AdventOfCode.task_14_1("src/main/resources/task_14_input.txt"))
}
...
object AdventOfCode {
  ...
  private def task_12_getPositionOf(heightmap: Array[Array[Char]], c: Char): (Int, Int) =
  ...
  private def task_12_getHeightAt(heightmap: Array[Array[Char]], position: (Int, Int)): Char =
  ...
  private def task_13_parsePackets(source: BufferedSource): List[List[Either[Char, Int]]] =
    source
      .getLines()
      .filterNot(_.isEmpty)
      .map(line =>
  ...
  private def task_13_isLessThen: (List[Either[Char, Int]], List[Either[Char, Int]]) => Boolean =
    (packet_1, packet_2) =>
      var p_1 = packet_1
      var p_2 = packet_2
      var p1_idx = 0
  ...
  def task_13_1(inputFile: String): Int =
    Using(Source.fromFile(inputFile)) { source =>
      val packets = task_13_parsePackets(source)

      var idx = 0
      var pairsInOrder = List[Int]()
      while (idx < packets.size) {
        if (task_13_isLessThen(packets(idx), packets(idx + 1))) {
  ...
  def task_13_2(inputFile: String): Int =
    Using(Source.fromFile(inputFile)) { source =>
      val dividerPacket_1 =
      ...
      val packets = (task_13_parsePackets(source) :+ dividerPacket_1 :+ dividerPacket_2)
        .sortWith(task_13_isLessThen)
  ...
  def task_14_1(inputFile: String): Int =
